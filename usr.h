...
void usr()/* 显示管理员管理系统选择界面 */
{
	...
	else
	{
		...
		//第一种方式:直接执行 
		UCHAR sql_0[MAXBUFLEN] = "select * from usr";
		...
		SQLBindCol(hstmt, 2, SQL_C_CHAR, list2, 32, 0);
		...
		do{
		  ...
		  if (strcmp(id, list1) == 0 && strcmp(md, list2) == 0){
			  printf("登陆成功\n");
			  strcpy(list3, list1);
		...
		do{
			...
			while (1)
			{
				...
				case '0':{
							 ...
							 SQLBindCol(hstmt, 2, SQL_C_CHAR, b, 30, 0);
				...
				case '1':{
							 ...
							 UCHAR sql_1[MAXBUFLEN] = "update usr set real_name = ?, work_id = ?, gender= ?, age = ? where usr_id = ?;";
							 SQLINTEGER p = SQL_NTS;
							 //1预编译   
							 SQLPrepare(hstmt, sql_1, 128);
							 //2 绑定参数值   
							 SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_CHAR, 200, 0, &c, 0, &p);
							 SQLBindParameter(hstmt, 2, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_CHAR, 200, 0, &d, 0, &p);
							 SQLBindParameter(hstmt, 3, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_CHAR, 200, 0, &e, 0, &p);
							 SQLBindParameter(hstmt, 4, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_CHAR, 200, 0, &f, 0, &p);
							 SQLBindParameter(hstmt, 5, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_CHAR, 200, 0, &list3, 0, &p);
							 //3 执行  
							 SQLExecute(hstmt);
				...
				case '6':{
							 ...
							 gets(a);
							 resultCode = SQLAllocHandle(SQL_HANDLE_STMT, hdbc, &hstmt);
							 UCHAR sql_5[MAXBUFLEN] = "update buy_book_list set buy_state = 'returned' where buy_state = 'unpaid' and book_id = ?;";
							 SQLINTEGER p = SQL_NTS;
							 //1预编译   
							 SQLPrepare(hstmt, sql_5, 128);
							 //2 绑定参数值   
